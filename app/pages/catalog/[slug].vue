...
const floors = computed(() =>
  [...new Set(units.value.map((u) => u.floor))].sort((a, b) => a - b),
);
...
const activePhoto = computed(
  () => photos.value[mainView.value] ?? photos.value[0],
);
...
const floorUnits = computed(() =>
  units.value.filter(
    (u) => u.floor === activeFloor.value && !PUBLIC_HIDDEN.includes(u.status),
  ),
);
const floorVacant = computed(() =>
  floorUnits.value.filter((u) => u.status === "VACANT"),
);
  ...
  return [
    {
      ...
      value: t("unitOf.sqmValue", { value: num(total, 1) }),
      tone: "text-ink-900",
    },
    {
      label: t("kpi.vacantArea"),
      value: t("unitOf.sqmValue", { value: num(vacant, 1) }),
      tone: "text-ok-700",
    },
    {
      label: t("kpi.occupiedArea"),
      value: t("unitOf.sqmValue", { value: num(total - vacant, 1) }),
      tone: "text-brand-700",
    },
  ];
});
...
const planBox = computed(
  () => `0 0 ${plan.value.width.toFixed(2)} ${plan.value.height.toFixed(2)}`,
);
...
function points(u: Unit) {
  ...
  return u.polygon
    .map(
      ([x, y]) =>
        `${((x ?? 0) * p.width).toFixed(2)},${((y ?? 0) * p.height).toFixed(2)}`,
    )
    .join(" ");
}
...
const infoRows = computed(() => {
  ...
  return [
    {
      icon: "building",
      label: t("field.buildingClass"),
      value: buildingClassLabel(b.buildingClass),
    },
    {
      icon: "calendar",
      label: t("field.buildYear"),
      value: t("cat.yearValue", { year: b.buildYear }),
    },
    {
      icon: "layers",
      label: t("kpi.totalArea"),
      value: t("unitOf.sqmValue", { value: num(b.gla) }),
    },
    {
      icon: "dashboard",
      label: t("field.floorCount"),
      value: b.undergroundFloors
        ? t("cat.floorsWithUnderground", {
            floors: b.floors,
            under: b.undergroundFloors,
          })
        : t("cat.floorsValue", { floors: b.floors }),
    },
    {
      icon: "cube",
      label: t("cat.parking"),
      value: b.undergroundFloors
        ? t("cat.parkingBoth", { floors: b.undergroundFloors })
        : t("cat.parkingSurface"),
    },
    { icon: "clock", label: t("field.slaLevel"), value: `${b.sla}%` },
  ];
});
...
const mapStats = computed(() => {
  ...
  return [
    {
      label: t("kpi.vacantUnits"),
      value: `${b.vacantUnits} ${t("unitOf.pcs")}`,
    },
    {
      label: t("field.buildingClass"),
      value: buildingClassLabel(b.buildingClass),
...
const mapZoom = computed(() => {
  ...
  if (!b) return 12;
  const inner =
    b.lon >= 69.19 && b.lon <= 69.4 && b.lat >= 41.25 && b.lat <= 41.38;
  return inner ? 14 : 12;
});
...
const priceInfo = computed(() => {
  ...
  return {
    perLabel: monthly ? t("cat.perSqmMonthly") : t("cat.perSqm"),
    perValue: money(Math.round(monthly ? u.price / u.area : u.price)),
    totalLabel: monthly ? t("cat.monthlyPayment") : t("cat.totalPrice"),
    totalValue: money(Math.round(monthly ? u.price : u.price * u.area)),
  };
});
...
const unitFacts = computed(() => {
  ...
  if (!u) return [];
  const rows = [
    { label: t("kpi.totalArea"), value: area(u.area) },
    { label: t("field.floor"), value: floorLabel(u.floor) },
    { label: t("field.rooms"), value: `${u.rooms} ${t("unitOf.pcs")}` },
  ];
  if (u.usage)
    rows.push({ label: t("field.purpose"), value: unitUsageLabel(u.usage) });
  return rows;
});
...
<template>
  <div class="mx-auto max-w-[1360px] px-4 py-6 lg:px-8 lg:py-8">
    <template v-if="building">
      ...
      <header class="mt-2 flex flex-wrap items-end justify-between gap-4">
        <div class="min-w-0">
          ...
          >
            ...
            <span class="text-ink-300">·</span>
            {{ buildingClassLabel(building.buildingClass) }}
          </p>
        </div>
      ...
      <div class="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div class="min-w-0 space-y-5">
          <section
            class="rounded-card bg-surface p-4 shadow-card ring-1 ring-ink-200/60 sm:p-5"
          >
            <UiPhoto
              v-if="activePhoto"
            ...
            >
              ...
              <span
                class="absolute bottom-4 left-4 text-[13px] font-semibold text-white"
              >
                {{ activePhoto.label }}
              </span>
            ...
            <div
              ...
              class="mt-3 grid grid-cols-4 gap-2.5 sm:grid-cols-5"
            >
              <button
                v-for="(p, i) in photos"
                :key="p.name"
          ...
          >
            <h2 class="text-[18px] font-bold">{{ t("cat.passport") }}</h2>
            <dl class="mt-4 grid gap-x-6 sm:grid-cols-2">
              ...
              >
                ...
                <dd
                  class="tabular max-w-[52%] text-right text-[14px] font-semibold text-ink-900"
                >
                  {{ r.value }}
                </dd>
              </div>
          ...
          <section
            class="rounded-card bg-surface p-5 shadow-card ring-1 ring-ink-200/60"
          >
            <h2 class="text-[18px] font-bold">{{ t("cat.amenities") }}</h2>
          ...
          <section
            class="rounded-card bg-surface p-5 shadow-card ring-1 ring-ink-200/60"
          >
            <h2 class="text-[18px] font-bold">{{ t("cat.floorsSection") }}</h2>
            ...
            <template v-if="floors.length">
              ...
              <div
                class="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,268px)]"
              >
                <div class="min-w-0">
                  <div
                    class="rounded-card bg-surface-sunken p-3 ring-1 ring-inset ring-ink-200"
                  >
                    <svg
                      :viewBox="planBox"
                      class="block w-full"
                      role="group"
                      :aria-label="t('cat.planAria', { floor: activeFloor })"
                    >
                      <rect
                        x="0"
                        y="0"
                      ...
                      <g
                        ...
                        :aria-label="
                          t('cat.unitAria', {
                            code: u.code,
                            status: statusLabel('unit', u.status),
                            area: area(u.area),
                          })
                        "
                ...
                <div class="min-w-0 space-y-4">
                  <dl
                    class="rounded-card bg-surface-sunken p-4 ring-1 ring-inset ring-ink-200"
                  >
                    <div
                    ...
                    >
                      ...
                      <dd class="tabular text-[13px] font-bold" :class="s.tone">
                        {{ s.value }}
                      </dd>
                    </div>
                  </dl>
                  ...
                  <div>
                    <p class="text-[13px] font-bold text-ink-900">
                      ...
                      <span class="tabular font-semibold text-ink-500">
                        {{
                          t("login.certCount", { count: floorVacant.length })
                        }}
                      </span>
                    </p>
          ...
          >
            ...
            <div
              class="mt-4 overflow-hidden rounded-card ring-1 ring-inset ring-ink-200"
            >
              <UiMap
                :markers="mapMarkers"
        ...
        <div class="min-w-0">
          ...
          >
            ...
            <div class="flex items-center justify-between gap-3">
              ...
              <div class="flex items-center gap-2">
                <button
                  ...
                  :aria-label="
                    t('cat.favouriteUnitAria', { code: selected.code })
                  "
                ...
                >
                  <svg
                    ...
                    viewBox="0 0 24 24"
                    :fill="
                      favourites.includes(selected.id) ? 'currentColor' : 'none'
                    "
                    stroke="currentColor"
                    stroke-width="1.7"
                    aria-hidden="true"
            ...
            <p class="mt-1 text-[13px] text-ink-500">
              {{ building.name }} · {{ floorLabel(selected.floor) }}
            </p>
            ...
            <template v-if="selected.status === 'VACANT' && priceInfo">
              ...
              <div class="mt-4">
                ...
                <p
                  class="tabular mt-1 text-[28px] font-extrabold leading-none text-brand-700"
                >
            ...
            <template v-if="selected.equipment.length">
              <h3 class="mt-4 text-[13px] font-bold text-ink-900">
                {{ t("cat.equipment") }}
              </h3>
            ...
            <!--
              Sotuvdagi maydonda ariza tugmasi yo'q: oldi-sotdi shartnomasi
              tizim orqali tuzilmaydi, shuning uchun mijoz bevosita sotuv
              bo'limi bilan bog'lanadi.
            -->
            <template
              v-if="selected.status === 'VACANT' && selected.offer === 'Sotuv'"
            >
              <UiButton block size="lg" class="mt-5" :to="CONTACT.phoneHref">
      ...
      <UiModal
        ...
        :title="activePhoto?.label ?? t('cat.gallery')"
        :subtitle="`${building.name}, ${t('common.of', { current: mainView + 1, total: photos.length })}`"
        size="xl"
      >
    ...
    >
      ...
      <p
        class="mx-auto mt-2 max-w-[46ch] text-[14px] leading-relaxed text-ink-600"
      >
        {{ t("cat.notFoundText") }}
      </p>
