...
const checklist = computed(() =>
  order.value ? checklistFor(order.value) : [],
);
...
const evidenceCount = computed(() =>
  order.value ? (evidence.value[order.value.id] ?? 0) : 0,
);
const orderNotes = computed(() =>
  order.value ? (notes.value[order.value.id] ?? []) : [],
);
...
const materials = computed(() =>
  order.value ? materialsFor(order.value.code) : [],
);

/** Taxminiy xarajat: topshiriq bo‘yicha rejalashtirilgan material qiymati */
const estimate = computed(() =>
  order.value ? materialsTotal(order.value.code) : 0,
);
...
const issuedRequest = computed(() =>
  materialRequests.value.find(
    (m) => m.workOrder === order.value?.code && m.status === "ISSUED",
  ),
);
...
const actReady = computed(
  () => !!order.value && ACT_STATUSES.includes(order.value.status),
);
...
function saveEvidence() {
  ...
  evidence.value[o.id] =
    (evidence.value[o.id] ?? 0) + evidenceFiles.value.length;
  notes.value[o.id] = [
...
<template>
  <AppTopbar
    ...
    :subtitle="
      order ? `${order.code} · ${order.buildingName}` : t('wo.notFoundSubtitle')
    "
    :breadcrumb="[
          ...
          <div class="mt-5 border-t border-ink-100 pt-5">
            ...
            <dl class="mt-3 grid gap-x-5 gap-y-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div class="min-w-0">
                ...
                >
                  {{ dateShort(order.createdAt.slice(0, 10)) }}
