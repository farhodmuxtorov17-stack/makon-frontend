...
const HERO_STATS = computed(() => [
  { label: t('landing.statObjects'), value: num(PORTFOLIO_TOTALS.buildings), unit: '' },
  { label: t('landing.statVacantUnits'), value: num(vacantCount.value), unit: '' },
  {
...
const mapStats = computed(() => [
  { label: t('landing.mapStatObjects'), value: num(PORTFOLIO_TOTALS.buildings) },
      ...
      <div class="relative mx-auto max-w-[1200px] px-4 pb-16 pt-14 lg:px-8 lg:pb-24 lg:pt-24">
        ...
        >
          <div class="grid gap-2.5 sm:grid-cols-[minmax(0,1fr)_190px_auto]">
            ...
            <UiSelect v-model="fType" :options="TYPE_OPTIONS" :aria-label="t('landing.fieldType')" />
        ...
        <dl class="mt-12 grid max-w-[840px] grid-cols-3 gap-6 border-t border-white/15 pt-7">
          <div v-for="s in HERO_STATS" :key="s.label">
            ...
            <dt class="mt-2 text-[12px] font-medium leading-snug text-white/60">{{ s.label }}</dt>
          </div>
        </dl>
      </div>
    </section>
          ...
          >
            <div class="relative">
              ...
              <span
                ...
                :class="l.unit.offer === 'Sotuv' ? 'bg-teal-700' : 'bg-brand-500'"
              >
    ...
    <section class="border-y border-ink-200/80 bg-surface">
      <div class="mx-auto max-w-[1200px] px-4 py-16 lg:px-8 lg:py-24">
        ...
        <ol class="mt-10 grid gap-x-10 gap-y-9 lg:grid-cols-3">
          <li v-for="s in STEPS" :key="s.step" class="border-t-2 border-brand-500 pt-5">
            <p class="tabular text-[12px] font-bold uppercase tracking-[0.09em] text-brand-600">
              {{ t('landing.stepNo', { step: s.step }) }}
            </p>
            <h3 class="mt-2.5 text-[18px] font-bold">{{ t(s.titleKey) }}</h3>
            <p class="mt-2 max-w-[44ch] text-[14px] leading-relaxed text-ink-600">
              {{ t(s.textKey) }}
            </p>
          </li>
        </ol>
