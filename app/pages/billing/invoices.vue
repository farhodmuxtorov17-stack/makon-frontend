...
const { periodLabel } = useAppLabels();
...
const paidShare = computed(() =>
  summary.value.charged
    ? Math.round((summary.value.paidTotal / summary.value.charged) * 100)
    : 0,
);
...
const period = ref(
  requestedPeriod && INVOICES.some((i) => i.period === requestedPeriod)
    ? requestedPeriod
    : "all",
);
...
const scoped = computed(() =>
  INVOICES.filter((i) => {
    ...
    const byQuery =
      !q ||
      `${i.code} ${i.tenant} ${i.buildingName} ${i.unitCode}`
        .toLowerCase()
        .includes(q);
    const byBuilding =
      building.value === "all" || i.buildingName === building.value;
...
const filtered = computed(() =>
  scoped.value.filter(
    (i) => status.value === "all" || i.status === status.value,
  ),
);
...
const columns = computed(() => [
  ...
  {
    key: "total",
    label: field("totalAmount", "Jami summa"),
    align: "right" as const,
    numeric: true,
  },
  {
...
const filteredTotal = computed(() =>
  filtered.value.reduce((s, i) => s + i.total, 0),
);
const filteredBalance = computed(() =>
  filtered.value.reduce((s, i) => s + (i.total - i.paid), 0),
);
...
const active = computed(
  () => INVOICES.find((i) => i.id === activeId.value) ?? null,
);
  ...
  payments.value = [
    {
      ...
      methodLabel:
        methodOptions.value.find((m) => m.value === payMethod.value)?.label ??
        payMethod.value,
      account: "",
...
const pending = computed<PendingRow[]>(() => {
  ...
  const berilgan = new Set(
    INVOICES.filter((i) => i.period === davr).map(
      (i) => `${i.tenant}|${i.unitCode}`,
    ),
  );
...
const selected = computed(
  () => pending.value.find((p) => p.caseId === selectedCase.value) ?? null,
);
...
function createInvoice() {
  ...
  if (row.manba === "sikl") {
    ...
    INVOICES.unshift({
      ...seed,
      status: statusOf(seed),
      agingBucket: agingKeyOf(seed),
    });
    banner.value = t("bil.invoiceIssuedFor", {
      code,
      period: row.label,
      tenant: row.tenant,
    });
...
const agingTotal = computed(() =>
  aging.value.reduce((s, a) => s + a.amount, 0),
);
...
<template>
  <AppTopbar
    ...
    :subtitle="
      moduleCaption('invoices', 'Davr bo‘yicha chiqarilgan hisob-fakturalar')
    "
    :breadcrumb="[
      {
  ...
  <main class="scroll-slim flex-1 space-y-5 overflow-y-auto p-4 sm:p-6">
    ...
    >
      ...
      >
        ...
        <template #cell-balance="{ row }">
          <span
            :class="
              Number(row.balance) > 0 ? 'text-danger-600' : 'text-ink-400'
            "
          >
      ...
      >
        ...
        <ul class="mt-5 divide-y divide-ink-100 border-t border-ink-100">
          ...
          >
            ...
            <span
              class="tabular w-12 text-right text-[13px] font-semibold text-ink-700"
            >
              {{ percent(a.share) }}
            </span>
            <span
              class="tabular w-40 text-right text-[13px] font-bold text-ink-900"
            >
              {{ money(a.amount) }}
            </span>
          </li>
            ...
            >
              ...
              <span class="min-w-0 flex-1">
                ...
                <span class="tabular block text-[12px] text-ink-500">
                  {{ t("common.countPcs", { n: num(p.count) }) }} ·
                  {{ percent(p.share) }}
                </span>
              </span>
    ...
    >
      ...
      <div
        class="grid gap-3 border-y border-ink-100 bg-surface-sunken px-5 py-3.5 sm:grid-cols-3"
      >
        <div>
          <p class="text-[12px] text-ink-500">
            {{ field("tenant", "Ijarachi") }}
          </p>
        ...
        <div>
          <p class="text-[12px] text-ink-500">
            {{ field("paymentTerm", "To‘lov shakli") }}
          </p>
          <p class="mt-0.5 text-[14px] font-semibold text-ink-900">
    ...
    <UiModal
      ...
      :subtitle="
        active ? `${active.tenant} · ${periodLabel(active.period)}` : ''
      "
    >
      <div v-if="active" class="space-y-6">
        ...
        <dl class="grid gap-4 sm:grid-cols-3">
          ...
          <div class="rounded-field bg-surface-sunken p-3.5">
            <dt class="text-[12px] text-ink-500">
              {{ field("paid", "To‘langan") }}
            </dt>
            <dd class="tabular mt-1 text-[16px] font-bold text-ok-600">
              {{ money(active.paid) }}
            </dd>
          </div>
          <div class="rounded-field bg-surface-sunken p-3.5">
            <dt class="text-[12px] text-ink-500">
              {{ field("balance", "Qoldiq") }}
            </dt>
        ...
        <dl
          class="grid gap-x-6 gap-y-3 border-t border-ink-100 pt-5 sm:grid-cols-2"
        >
        ...
        <section
          ...
          class="border-t border-ink-100 pt-5"
        >
          <p class="text-[14px] font-bold text-ink-900">
            {{ t("bil.paymentHistory") }}
          </p>
          <ul
            class="mt-3 divide-y divide-ink-100 rounded-field bg-surface-sunken px-4"
          >
            <li
            ...
            >
              ...
              <span class="min-w-0 flex-1">
                ...
                <span class="block text-[12px] text-ink-500">
                  {{ p.purpose
                  }}<template v-if="p.account"> · {{ p.account }}</template>
                  <template v-if="p.actor"> · {{ p.actor }}</template>
                </span>
                <span
                  v-if="p.note"
                  class="mt-0.5 block text-[12px] text-ink-600"
                  >{{ p.note }}</span
                >
              </span>
              <span
                ...
                :class="
                  p.kind === 'payment' ? 'text-ok-600' : 'text-danger-600'
                "
              >
        ...
        <div
          ...
          class="rounded-card bg-surface-sunken p-4 ring-1 ring-ink-200/70"
        >
          <p class="text-[14px] font-bold text-ink-900">
            {{ t("bil.acceptPayment") }}
          </p>
          <div class="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <UiField :label="t('common.note')" :hint="t('bil.noteHint')">
              <UiInput
                v-model="payNote"
                :placeholder="t('bil.notePlaceholder')"
              />
            </UiField>
          </div>
          <p
            v-if="!payValid"
            class="mt-3 text-[12px] font-medium text-danger-600"
          >
      ...
      <template #footer>
        <UiButton variant="ghost" @click="detailOpen = false">{{
          t("common.close")
        }}</UiButton>
        <UiButton
          v-if="canConfirm && payable"
    ...
    >
      <p
        v-if="!pending.length"
        class="text-[14px] leading-relaxed text-ink-600"
      >
      ...
      <template v-else>
        ...
        <dl
          v-if="selected"
          class="mt-4 rounded-field bg-surface-sunken px-4 py-3.5"
        >
          ...
          <div
            class="mt-3 flex items-baseline justify-between gap-3 border-t border-ink-100 pt-3"
          >
