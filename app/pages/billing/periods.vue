...
const periodKeys = computed(() => {
  ...
  for (const key of rangeKeys(
    earliest && earliest < CURRENT_KEY ? earliest : CURRENT_KEY,
    CURRENT_KEY,
  )) {
    keys.add(key);
  }
  ...
  () =>
    new Map(
      periodKeys.value.map((key) => [
        key,
        [startOfKey(key), endOfKey(key)] as const,
      ]),
    ),
);
...
const stateTabs = computed(() => [
  { value: "all", label: t("tab.all"), count: periods.value.length },
  {
    value: "open",
    label: t("tab.open"),
    count: periods.value.filter((p) => !p.closed).length,
  },
  {
    value: "closed",
    label: t("tab.closed"),
...
const totalCharged = computed(() =>
  periods.value.reduce((s, p) => s + p.total, 0),
);
const openCount = computed(() => periods.value.filter((p) => !p.closed).length);
const closedCount = computed(
  () => periods.value.filter((p) => p.closed).length,
);
const generatedCount = computed(() =>
  periods.value.reduce((s, p) => s + p.invoices, 0),
);
...
const areaByBuilding = computed(() => {
  ...
  for (const u of UNITS)
    map.set(u.buildingId, (map.get(u.buildingId) ?? 0) + u.area);
  return map;
});
...
function invoiceTotalOf(contract: Contract): number {
  ...
  const services = tariffLinesFor(
    METERS,
    contract.buildingName,
    unit.area,
  ).reduce(
    (s, line) =>
      s + (line.meter ? Math.round(line.tariff * line.qty * share) : line.sum),
    0,
  );
...
function generate(key: string): Invoice[] {
  ...
  const covered = new Set(
    (invoicesByKey.value.get(key) ?? []).map((i) => i.contractCode),
  );
...
const target = computed(
  () => periods.value.find((p) => p.key === targetKey.value) ?? null,
);
...
function applyConfirm() {
  ...
  if (confirmMode.value === "generate") {
    ...
    if (!closedKeys.value.includes(p.key))
      closedKeys.value = [...closedKeys.value, p.key];
...
const detail = computed(
  () => periods.value.find((p) => p.key === detailKey.value) ?? null,
);
...
const yearOptions = computed(() =>
  [0, 1].map((offset) => {
    ...
    return {
      value: String(year),
      label: t("bil.yearLabel", { year: String(year) }),
    };
  }),
);
...
function createPeriod() {
  ...
  banner.value = t("bil.periodOpenedBanner", {
    period: labelOfKey(newKey.value),
  });
...
<template>
  <AppTopbar
    ...
    :subtitle="
      moduleCaption('periods', 'Hisob-kitob davrlarini ochish va yopish')
    "
    :breadcrumb="[
      {
  ...
  <main class="scroll-slim flex-1 space-y-5 overflow-y-auto p-4 sm:p-6">
    ...
    >
      ...
      >
        ...
        <template #cell-actions="{ row }">
          <span v-if="!canIssue" class="text-[12px] text-ink-500">{{
            t("bil.viewOnly")
          }}</span>
    ...
    <UiCard
      :title="t('bil.chargedByPeriod')"
      :subtitle="t('bil.chargedByPeriodCaption')"
    >
      <UiBars
        :labels="chartLabels"
        :series="chartSeries"
    ...
    <UiModal
      ...
      "
      :subtitle="
        target ? t('bil.periodSubtitle', { period: target.label }) : ''
      "
    >
      <div v-if="target" class="space-y-4">
        ...
        <dl
          class="divide-y divide-ink-100 rounded-field bg-surface-sunken px-4"
        >
          <div class="flex items-baseline justify-between gap-4 py-2.5">
            <dt class="text-[13px] text-ink-500">
              {{ t("kpi.activeContracts") }}
            </dt>
    ...
    >
        ...
        <div class="flex items-center gap-3">
          ...
          <span class="text-[13px] text-ink-500">
            ...
            {{
              percent(
                detail.contracts
                  ? (detail.invoices / detail.contracts) * 100
                  : 0,
              )
            }}
          </span>
      ...
      <template #footer>
        ...
        <UiButton
          :to="`/billing/invoices?period=${detail ? detail.label : ''}`"
        >
          <UiIcon name="doc" :size="16" />
          {{ t("bil.viewInvoices") }}
        </UiButton>
