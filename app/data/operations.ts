...
export const SERVICE_REQUESTS: ServiceRequest[] = [
  ...
  },
  {
    ...
    slaBreached: false,
    description:
      "Sanuzeldagi quvur oqishini bartaraf etish va aralashtirgichni almashtirish.",
    progress: 100,
  ...
  },
  {
    ...
    slaBreached: true,
    description:
      "Zinapoyalarda avariya yoritgichlarini o‘rnatish va datchiklarni sozlash.",
    progress: 45,
  },
  {
    id: "s-0685",
  ...
  },
  {
    ...
    slaBreached: false,
    description:
      "Kirish eshigining qulfi yaxshi yopilmayapti, mexanizmni sozlash kerak.",
    progress: 55,
  },
];
...
export type ServiceKpiTone =
  "brand" | "ok" | "warn" | "danger" | "violet" | "neutral";
...
const SERVICE_GROUPS: Array<{
  ...
  { label: "Yangi", tone: "brand", statuses: ["NEW"] },
  {
    label: "Saralashda",
    tone: "violet",
    statuses: ["TRIAGE", "ASSIGNED", "INSPECTION"],
  },
  {
    label: "Jarayonda",
    tone: "warn",
    statuses: ["MATERIAL_PENDING", "IN_PROGRESS"],
  },
  {
    label: "Bajarilgan",
    tone: "ok",
...
export function buildServiceKpi(
  list: ServiceRequest[] = SERVICE_REQUESTS,
): ServiceKpi {
  ...
  const avgHours = finished.length
    ? Math.round(
        (finished.reduce(
          (s, r) => s + hoursBetween(r.createdAt, r.completedAt!),
          0,
        ) /
          finished.length) *
          10,
 ...
 */
export function buildStockCategories(
...
const WORK_MATERIAL_REFS: Record<
  string,
...
export function issueLinesFor(
  orderCode: string,
): Array<{ name: string; unit: string; qty: number }> {
  return materialsFor(orderCode).map((m) => ({
    name: m.name,
    unit: m.unit,
    qty: m.qty,
  }));
...
export const WORK_CHECKLIST: WorkChecklistItem[] = checklistFor(
  serviceRequestByCode(DEFAULT_ORDER_CODE) ?? {
    category: "Elektr",
    progress: 0,
  },
);

export const WORK_MATERIALS: WorkMaterialLine[] =
  materialsFor(DEFAULT_ORDER_CODE);
...
export interface MaterialRequest {
  ...
  amount: number;
  status:
    "DRAFT" | "SUBMITTED" | "APPROVED" | "REJECTED" | "ISSUED" | "CANCELLED";
  createdAt: string;
  buildingName: string;
}
...
export const MATERIAL_REQUESTS: MaterialRequest[] = MATERIAL_REQUEST_SEED.map(
  (seed) => {
...
export function buildWarehouseSummary(
  ...
  const lastReceiptDay = list.reduce(
    (d, i) => (i.addedAt > d ? i.addedAt : d),
    "",
  );
  return {
    inbound: list
      .filter((i) => i.addedAt === lastReceiptDay)
      .reduce((s, i) => s + i.qty, 0),
    outbound: requests
      .filter((r) => r.status === "ISSUED")
      .reduce(
        (s, r) => s + issueLinesFor(r.workOrder).reduce((q, l) => q + l.qty, 0),
        0,
      ),
...
export function daysToVerification(
  meter: Meter,
...
export function isVerificationSoon(
  meter: Meter,
  today: string = todayIso(),
): boolean {
...
export function meterStateLabel(
  meter: Meter,
  today: string = todayIso(),
): string {
...
export function buildUtilitySummary(
  ...
  return UTILITY_TYPES.flatMap(({ type, label, icon }) => {
    ...
    const decimals = rows.every(
      (m) =>
        Number.isInteger(m.lastReading) && Number.isInteger(m.previousReading),
    )
      ? 0
      : 1;
    return [
...
export interface AppNotification {
  id: string;
  /*
   * Sarlavha va matn tarjima kaliti bilan uzatiladi: bu fayl ma'lumot
   * qatlami, u tilni bilmaydi. Ekran `useNotificationText()` orqali
   * tanlangan tildagi matnni oladi, `title` va `body` esa zaxira qiymat
   * bo'lib qoladi (hujjat generatsiyasi va qidiruv uchun).
   */
  titleKey: string;
  bodyKey: string;
...
export function paymentAlerts(today: string = todayIso()): {
  ...
  const due =
    upcoming[0] ?? [...open].sort((a, b) => b.daysLeft - a.daysLeft)[0] ?? null;
  const overdue =
    open
...
function dueBody(a: InvoiceAlert): string {
  ...
  if (a.daysLeft === 0)
    return `${a.code} hisob-fakturasi bo‘yicha to‘lov muddati bugun tugaydi.`;
...
export function buildNotifications(
  ...
  if (due) {
    list.push({
      id: "n-01",
      titleKey: due.daysLeft >= 0 ? "notif.dueSoonTitle" : "notif.duePastTitle",
      bodyKey:
        due.daysLeft > 0
          ? "notif.dueInDays"
          : due.daysLeft === 0
      ...
      params: {
        code: due.code,
        days: Math.abs(due.daysLeft),
        date: dateShort(due.dueAt),
      },
      title:
        due.daysLeft >= 0
  ...
  if (overdue) {
    list.push({
      id: "n-05",
      titleKey: "notif.overdueTitle",
      bodyKey: "notif.overdueBody",
      params: {
        tenant: overdue.tenant,
        code: overdue.code,
        amount: num(overdue.debt),
        days: -overdue.daysLeft,
      },
  ...
  list.push({
    id: "n-06",
    titleKey: "notif.newDeviceTitle",
    bodyKey: "notif.newDeviceBody",
  ...
  list.push({
    ...
    id: "n-03",
    titleKey: "notif.unitVacantTitle",
    bodyKey: "notif.unitVacantBody",
  ...
  list.push({
    ...
    id: "n-04",
    titleKey: "notif.didoxSigningTitle",
    bodyKey: "notif.didoxSigningBody",
  ...
  const finished = SERVICE_REQUESTS.find(
    (r) => r.status === "COMPLETED" && !!r.completedAt,
  );
  if (finished?.completedAt) {
    list.push({
      id: "n-02",
      titleKey: "notif.serviceDoneTitle",
      bodyKey: "notif.serviceDoneBody",
      params: { code: finished.code, title: finished.title },
