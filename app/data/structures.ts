...
export const STRUCTURE_KIND: Record<StructureKind, StructureKindMeta> = {
  ...
  parkingUnderground: {
    label: "Yer osti avtoturargoh",
    short: "Yer osti parkovka",
    leasable: false,
    icon: "cube",
    tone: "neutral",
  },
  checkpoint: {
    label: "Nazorat-o‘tkazish punkti",
    short: "KPP",
    leasable: false,
    icon: "shield",
    tone: "neutral",
...
function composeSite(b: Building): Structure[] {
  ...
  const add = (s: Omit<Structure, "id" | "buildingId">) => {
    seq += 1;
    out.push({
      id: `${b.id}-s${String(seq).padStart(2, "0")}`,
      buildingId: b.id,
      ...s,
    });
  };
