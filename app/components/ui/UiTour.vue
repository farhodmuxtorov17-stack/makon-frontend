...
function place() {
  ...
  if (!b) {
    ...
    let top =
      below + h + EDGE <= vh ? below : above >= EDGE ? above : vh - h - EDGE;
...
/** Bu bo‘lim yo‘llanmasi ko‘rilgan deb belgilanadi */
function markSeen() {
...
onMounted(() => {
  if (props.auto && props.steps.length && !seen.value.includes(props.id)) {
    /*
     * Yo'llanma bo'limga birinchi marta kirilganda bir marta ochiladi va
     * shu zahoti «ko'rilgan» deb belgilanadi.
     *
     * Ilgari belgi faqat oxirgi qadamdan keyin qo'yilardi: foydalanuvchi
     * oynani yarim yo'lda yopsa yoki boshqa sahifaga o'tib ketsa, belgi
     * qolmasdi va yo'llanma o'sha bo'limga har safar kirganda qaytadan
     * ochilaverardi. Kerak bo'lsa uni sarlavhadagi tugma bilan qayta
     * ochish mumkin.
     */
    markSeen();
    ...
    >
      ...
      >
        <div :key="index" class="tour-step">
          <p
            class="text-[11px] font-bold uppercase tracking-wide text-brand-600"
          >
            {{
