...
<template>
  <div v-if="ticket" class="space-y-4">
    <!-- Didox kartasi -->
    <div
      class="rounded-card bg-surface-sunken p-4 ring-1 ring-inset ring-ink-200 sm:p-5"
    >
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="flex items-start gap-3">
          ...
          <div class="min-w-0">
            ...
            <p class="tabular mt-0.5 text-[16px] font-bold text-ink-900">
              {{ ticket.docNumber }}
            </p>
          </div>
        </div>
      ...
      <dl class="mt-4 grid gap-x-6 sm:grid-cols-2">
        <div
          v-for="r in [
            ...
            {
              l: t('ui.sentAt'),
              v: `${dateShort(ticket.sentAt)} ${timeOf(ticket.sentAt)}`,
            },
        ...
        >
          ...
          <dd
            class="tabular min-w-0 text-right text-[13px] font-semibold text-ink-900"
          >
            {{ r.v }}
          </dd>
        </div>
    ...
    <div>
      <p
        class="mb-2.5 text-[12px] font-semibold uppercase tracking-wide text-ink-500"
      >
        {{ t("ui.stateHistory") }}
      </p>
      <ol class="relative space-y-4 pl-8">
        <span
          class="absolute bottom-2 left-[13px] top-2 w-px bg-ink-200"
          aria-hidden="true"
        />
        <li
